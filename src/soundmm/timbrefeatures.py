...
from . import timbretoolboxstats


class TimbreFeatures:
    def __init__(self, raw_df: pd.DataFrame):
        """
        Class for post-processing raw timbre features from AudioCommons (ac) Timbral Models and Timbre Toolbox (tt).
        """
        self.raw_df = raw_df
        ...
        # TODO provide an option to raise an error if any NaN is found
        ...
        for c in self.raw_tt_cols:
            ...
            if not (c.endswith('_min') or c.endswith('_max')):
                is_excluded = False
                for excluded_name in timbretoolboxstats.excluded_features:
        ...
        for col in self.tt_cols:
            ...
            # New (april 2023) log scales applied to energies
                    ...
                    dynamic_eps = timbretoolboxstats.log_scale_dynamic_eps[col.replace('tt_', '')]
            ...
            #       observe it improves the repartition of values)
                ...
                dynamic_eps = timbretoolboxstats.log_scale_dynamic_eps[col.replace('tt_', '')]
                self.postproc_df[col] = np.log(dynamic_eps + self.postproc_df[col])
        # Finally: normalize using pre-computed statistics
        #    TODO allow users to provide their own normalization statistics
        mean, std = pd.Series(_post_distorsion_stats['mean']), pd.Series(_post_distorsion_stats['std'])
        self.postproc_df[self.feature_cols] \
            = (self.postproc_df[self.feature_cols] - mean[self.feature_cols]) / std[self.feature_cols]
