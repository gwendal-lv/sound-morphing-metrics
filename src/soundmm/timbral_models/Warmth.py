...
def warm_region_cal(audio_data, fs):
    """
    ...
    """
    windowed_samples = audio_data['windowed_audio']
...
def timbral_warmth(audio_data: Dict[str, Any],
                   dev_output=False, clip_output=False, max_FFT_frame_size=8192,
                   max_WR = 12000):
    """
    ...
    """
    audio_samples, fs, windowed_samples = audio_data['audio_samples'], audio_data['fs'], audio_data['windowed_audio']

    # get the weighted high frequency content
    mean_wr, _, _, weighted_hf = warm_region_cal(audio_data, fs)
